...
template <typename T>
using t_list = struct s_list<T>; // alias template - name that refers to a previously defined type
...
class Stack {
 private:
  ...
  std::size_t size = 0;
 public:
  void init() { _top = new t_list<T>; } //? Что значит создание пустого стека?
  void push(T value);
  T pop();
  T peek() {return _top->next != nullptr ? _top->next->content : _top->content;}
